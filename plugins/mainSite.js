...
export default async function plugin({ succeed, fail, chalk: { white }, tool }, options = {}) {
  let {
    mainSite: { url, httpsInfo, reps, isRedirect },
    option,
  } = options
  let { rawHeaders } = reps
  ...
  //检测index.html压缩方式
  try {
    let res = await request({ url, headers: { 'Accept-Encoding': 'gzip, deflate, br' }, m: option.min })
  ...
  DIS.setMainSite({ url, hsts, mainSiteZip: zip, https, crossDomain, mainSiteHttp2: http2, isRedirect, mainSiteCache: cache })
