function checkCrossDomain(rawHeaders) {
  ...
  if (acaoIdx === -1) {
    res = { code: 0, msg: `未配置跨域规则` }
  } else {
    if (rawHeaders[acaoIdx + 1] === '*') {
      res = { code: 0, msg: `跨域配置为 *,请检查` }
    } else {
      res = { code: 1, msg: `跨域配置暂无问题` }
    }
  }
...
function checkServer(rawHeaders) {
  let idx = -1
  for (let i in rawHeaders) {
    if (rawHeaders[i].toString() === 'Server') {
      idx = Number(i)
    }
  }
  if (idx !== -1) {
    return { code: 1, msg: `服务器部署软件为  ${rawHeaders[idx + 1]}` }
  } else {
    return { code: 0, msg: '未检测出服务器部署软件' }
  }
}
