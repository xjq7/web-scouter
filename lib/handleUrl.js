...
function handleUrl(data) {
  const { sourceCheckBlackListFilter, sourceCheckWhiteListFilter, filterStaticResource, blackMiniChunkSourceFilter } = filter
  let { url } = DIS.getMainSite()
  const reg = /(?:href|src)=((["']?)((https:\/\/)?)(.*?)(\.js|\.css|\.ico|>))/g
  let urls = []
  let regRes = data.match(reg)
  // console.log(regRes)
  //没有协议就加上协议,白名单校验,只检测白名单中的
  if (regRes) {
    for (let v of regRes) {
