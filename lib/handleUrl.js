...
function saveRobotMsg({ cdn, cache, zip, sourceSize }) {
  DIS.setCache(cache)
  DIS.setCdn(cdn)
  DIS.setZip(zip)
...
async function fetchUrls(data) {
  const { checkCdn, checkCache, checkSourceSize, checkZip } = checkTool
  ...
  results.forEach((item, index) => {
    ...
    if (size / 1024 > 10 || urls[index].indexOf('app') !== -1) {
      ...
      //将检测信息存储等待发送到机器人
      saveRobotMsg({ cdn, cache, zip, sourceSize })
      //整个数组,每一项代表每个url的检测详情,注入到每个plugin中
      res.push({ name: deHashName, url: urls[index], cdn, cache, zip, sourceSize, rawHeaders })
    }
  })
  return res
}
